...
class Docker::Event
  ...
  attr_accessor :status, :id, :from, :time, :actor, :actor_type

  def initialize(status, id, from, time, actor, actor_type)
    @status, @id, @from, @time, @actor, @actor_type = status, id, from, time, actor, actor_type
  end
